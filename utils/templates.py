...
def number_input_set(x,y,width,height,rotation,pack_number):
    x_temp = st.number_input("X-coordinate:", value = x, key = 1+(pack_number*5))
    y_temp = st.number_input("Y-coordinate:", value = y, key = 2+(pack_number*5))
    width_temp = st.number_input("Width:", value = width, key = 3+(pack_number*5))
    height_temp = st.number_input("Height:", value = height, key = 4+(pack_number*5))
    rotation_temp = st.number_input("Rotation:", value = rotation, key = 5+(pack_number*5))
    return x_temp, y_temp, width_temp, height_temp, rotation_temp


def custom(filename, images):
    ...
    x_1, y_1, width_1, height_1, rotation_1 = number_input_set(100, 100, 100, 100, 0, 0)

    # Cut the image
    cropped_image_1 = cut_image(images[page_number - 1], x_1, y_1, width_1, height_1, rotation_1)
    ...
    with st.sidebar:
        if st.button("Download Cropped Images", type='primary'):

            # Save the image to a temporary file
            cropped_image_1.save("Downloaded_Images/" + filename + "_cropped.png",format="PNG")

def dhl_parcel(filename, images):
    ...
    with col1:
        ...
        # Specify pixel dimensions for cutting
        x_1, y_1, width_1, height_1, rotation_1 = number_input_set(705, 150, 340, 340, 270, 0)
    ...
    with col2:
        ...
        # Specify pixel dimensions for cutting
        x_2, y_2, width_2, height_2, rotation_2 = number_input_set(1300, 50, 900, 1600, 270, 1)
    ...
    if international_parcel:
        with col3:
            ...
            # Specify pixel dimensions for cutting
            x_3, y_3, width_3, height_3, rotation_3 = number_input_set(510, 145, 635, 880, 0, 2)
